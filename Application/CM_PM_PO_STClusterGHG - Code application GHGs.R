...
setwd("C:/Users/Utente/OneDrive - Università degli Studi di Milano-Bicocca/Desktop/Paper_ClusteringGHG_SpaSta2025/Application/revision2")
setwd("C:/Users/Utente/OneDrive - Università degli Studi di Milano-Bicocca/Desktop/Paper_ClusteringGHG_SpaSta2025/Application")

#Data: emission per km2 by gas and by sector
load("SpaSta2025_SPTclu_GHG_Data_gas22.RData")
load("SpaSta2025_SPTclu_GHG_Data_sector22.RData")
load("SpaSta2025_SPTclu_GHG_Data_nuts2.RData")


load("C:/Users/Utente/OneDrive - Università degli Studi di Milano-Bicocca/Desktop/Paper_ClusteringGHG_SpaSta2025/Application/revision/res_gas22_rev.RData")
load("C:/Users/Utente/OneDrive - Università degli Studi di Milano-Bicocca/Desktop/Paper_ClusteringGHG_SpaSta2025/Application/revision/res_sector22_rev.RData")
res_gas22_rev<-res_gas22
res_sector22_rev<-res_sector22
  
load("C:/Users/Utente/OneDrive - Università degli Studi di Milano-Bicocca/Desktop/Paper_ClusteringGHG_SpaSta2025/Application/res_gas22.RData")
load("C:/Users/Utente/OneDrive - Università degli Studi di Milano-Bicocca/Desktop/Paper_ClusteringGHG_SpaSta2025/Application/res_sector22.RData")
...
reg_year_totals <- gas22 %>%
  select(NUTS, Unit, all_of(yrs)) %>%
  pivot_longer(cols = all_of(yrs), names_to = "year", values_to = "value") %>%
  group_by(NUTS, year) %>%
...
chg_by_region <- reg_year_totals %>%
  ...
  pivot_wider(names_from = year, values_from = total, names_prefix = "y_") %>%
  mutate(pct_change_1990_2022 = 100 * (y_2022 - y_1990) / y_1990) %>%
  select(NUTS, pct_change_1990_2022)
...
sptClu<- function(D_total, delta_alpha, K) {
  ...
  for (j in 1:num_matrices) {
    ...
    #delta_total[[j]] <- as.matrix(D_total[[j]])^2/(2 * n)
  }
  for (i in 1:length(alpha_combinations)) {
    ...
    # Also compute within-cluster dissimilarity for the combined dissimilarity matrix
    #W[i, num_matrices + 1] <- withindiss(D_combined, part)
    # Store the result
...
#cluster_color <- c("#BA59D3", "#602080", "#99008B", "#FF83FA")
#cluster_color <- c("#4C1D95",  # deep indigo
#                  "#7E22CE",  # vivid purple
...
joint_inertia<-function(d_tot,i,alpha_val,K){
  ...
  #old<-c(J_in_old, Qnorm_sp,Qnorm_ts, Q_sp_st, alpha_new)
  #new<-c(J_in, Qnorm_sp,Qnorm_ts_av, Q_sp_st, alpha_new)
  #return(c(J_in, Qnorm_sp,Qnorm_ts, Q_sp_st, alpha_new))
  #return(jin=J_in,qnorm=c(Qnorm_sp,Qnorm_ts),alpha_new=alpha_new,Q=c(Q_sp_st,Q_ts_st))
